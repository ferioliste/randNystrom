...
def block_short_axis(l, col, rc_local, col_random_seed, t):
	col_rng = np.random.default_rng(col_random_seed)
	
	sketch = np.zeros((rc_local, l), dtype='d')
	bounds = np.ceil(np.linspace(0,l,t+1))
	for i in range(rc_local):
		 col = col_rng.integers(bounds[:t], bounds[1:], size=t)
		 sketch[i,col] = col_rng.choice([-1, 1], size=t)*col_rng.uniform(1., 2., size=t)
	return sketch
...
save_results = False
save_svd = False
(n,l,k,sketch_matrix,t) = (None, None, None, None, None)
if rank == 0:
	line_id = get_counter()
	n, matrix_type, RR, p, sigma, l, k, sketch_matrix, t = get_settings_from_csv(line_id)
	print_settings(n, matrix_type, RR, p, sigma, l, k, sketch_matrix, t, s)
(n,l,k,sketch_matrix,t) = comm.bcast((n,l,k,sketch_matrix,t), root = 0)
...
row, col = rank%n_rowcol, rank//n_rowcol
...
match sketch_matrix:
	...
	case 1:
		omega_local = block_short_axis(l, col, rc_local, col_random_seed, t)
	case 2:
...
#======== block multiplications to obtain C and B ========
...
#======== computation of Q*U_k via reversed tree ========
if rank == 0:
